...
def go_left():
    ...
    # Uygulamaya tuş basımını gönderme.
pencere.listen()
...
while True:
    pencere.update() # while döngüsü bitmeden uygulama kapandığı için bazen hata veriyormuş gibi gösteriyor.
    ...
    if kafa.xcor() > 300 or kafa.xcor() < -300 or kafa.ycor() > 300 or kafa.ycor() < -300:
        ...
        # Kafa yemeğe değince kuyruk ekleme.
    if kafa.distance(yemek) < 20:
        x = random.randint(-250, 250)
        y = random.randint(-250, 250)
        yemek.goto(x, y)

        # Kuyruk kodları (beyaz kare).
